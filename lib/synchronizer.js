(function() {
  var Client, MetaContext, ResourceClient, SocketIO, Spine, Synchronizer,
  ...
  Synchronizer = (function(_super) {
    ...
    function Synchronizer(options) {
      ...
      this.appContext = options.context;
    ...
    Synchronizer.prototype.updateObject = function(uri, data) {
      if (this.appContext.exists(uri)) {
        return this.appContext.update(uri, data);
      } else {
        return this.appContext.create(uri, data, function(uri) {
          return console.log("created with uri", uri);
        });
      }
    };
